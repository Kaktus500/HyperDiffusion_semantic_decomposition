...
def get_model(cfg):
    if cfg.model_type == "mlp_3d":
        model = MLP3D_Split(**cfg.mlp_config)
...
def main(cfg: DictConfig):
    ...
    dataset_folder_path = HYPER_DIFF_DIR / Path(cfg.dataset_folder)
    ...
    curr_lr = cfg.lr
    root_path = HYPER_DIFF_DIR / Path(cfg.logging_root) / Path(cfg.exp_name)
    multip_cfg = cfg.multi_process
    files = [
        file
        for file in dataset_folder_path.iterdir()
        if file.name not in ["train_split.lst", "test_split.lst", "val_split.lst"]
    ]
    lengths = []
    names: List[str] = []
    train_object_names = np.genfromtxt(
        dataset_folder_path / "train_split.lst", dtype="str"
    )
    ...
    for i, file in enumerate(files):
        ...
        if file_name + ".obj" not in train_object_names:
            print(f"File {file} not in train_split")
            continue

        filename = file.name.split(".")[0]
        filename = f"{filename}_jitter_{0}"
        ...
        checkpoint_path = root_path / f"{filename}_model_final.pth"
        ...
        if (
            ...
            and (cfg.strategy == "first_weights" or cfg.strategy == "first_weights_kl")
