...
class MLP3D_Split(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i, _ in enumerate(hidden_neurons[:-1]):
            self.layers.append(
                SplitLinear(hidden_neurons[i], 4, 32, use_bias=use_bias) # TODO fixed size of blocks here
            )
        self.layers.append(nn.Linear(hidden_neurons[-1], out_size, bias=use_bias))
    ...
    def forward(self, model_input):
        ...
        if self.output_type == "occ":
            ...
            print("not implemented output type")
...
class SplitLinear(nn.Module):
    def __init__(
        ...
        for i in range(nr_of_blocks):
            self.layers.append(nn.Linear(in_size, size_of_blocks, bias=use_bias))
    def forward(self, model_input):
        output = [l(model_input) for l in self.layers]
